...
describe(scriptName + '4LTR (' + 'Instructor/Student' + ') :: STUDYBITS/PRACTICE QUIZ METRICS RECORDING ON INSTRUCTOR AND STUDENT CONCEPT TRACKER VALIDATION', function() {
	...
	it(". Record the metrics on student ConceptTracker for practice quiz", function(done) {
		this.retries(3);
	...
	it(". Store the correct and Total Questions answers after student's attempt on practice quiz from StudyBit", function(done) {
	...
	it(". Launch the Quiz", function(done) {
		...
		practiceQuizCreation.getQuizText(browser, productData.concepttracker.quiz.chapterbased.name)
			.then(function(practiceQuizResult) {
				...
				if (practiceQuizResult.indexOf(testBotQuizMetricsAtInstructor) > -1) {
					console.log(report.reportHeader() +
						report.stepStatusWithData("ANALYTICS :: Practice Quiz metrics retrieved from the platform after the student attempted a quiz on  \" " + productData.concepttracker.quiz.chapterbased.name + " Robo analytics " + testBotQuizMetricsAtInstructor + " is compared with ", practiceQuizResult, "success") + report.reportFooter());
					done();
				} else {
					console.log(report.reportHeader() +
						report.stepStatusWithData("ANALYTICS :: Practice Quiz metrics retrieved from the platform after the student attempted a quiz on  \" " + productData.concepttracker.quiz.chapterbased.name + " Robo analytics " + testBotQuizMetricsAtInstructor + " is compared with ", practiceQuizResult, "failure") + report.reportFooter());
				}
			});
	});
	...
	it(". Validate the count of StudyBits on instructor ConceptTracker after student's quiz attempts ", function(done) {
		...
		.then(function(status) {
			if (status === 1) {
				studybit.getStudyBitCountTextOnCT(browser, productData.concepttracker.quiz.studybitbased.name)
					.then(function(studybitCount) {
						var sbCountOnInstructorEndCount = parseInt(stringutil.returnValueAfterSplit(studybitCount, " ", 0));
						if (sbCountOnInstructorEndCount) {
							console.log(report.reportHeader() +
								report.stepStatusWithData("Studybit count on ConceptTracker", sbCountOnInstructorEndCount, "success") +
								report.reportFooter());
							done();
						} else {
							console.log(report.reportHeader() +
								report.stepStatusWithData("Studybit count on ConceptTracker", sbCountOnInstructorEndCount, "failure") +
								report.reportFooter());
						}
					});
			} else {
