...
var assignmentCount;
var recursiveFnPagepo = require("../../recursiveFnPagepo.js");
...
module.exports = {
  ...
  highscoreCheckboxByDefaultSelected: function (browser) {
    ...
    checkIfAssignmentSavedOnFuture2ndDate: function (browser, assignmentName) {
      var ast = "//div[@class='day ng-scope']/span/span[@class='number ng-binding' and (text()='2')]//parent::span//following-sibling::div[contains(@class,'event')]//span[contains(.,'{{}}')]";
      var res = stringutil.returnreplacedstring(ast, "{{}}", assignmentName);
        return browser
            .waitForElementByXPath(res, asserters.isDisplayed, 60000)
            .execute("return window.getComputedStyle(document.evaluate(\"" + res + "\", document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null ).singleNodeValue.parentNode).getPropertyValue('background-color');");
    ...
    // },
     ...
     deleteAssignment: function (browser, done) {
      ...
      .sleep(1000)
      .elementByCss(".icon-list-gray").click()
      .sleep(1000)
      .hasElementByXPath("//div[contains(@class,'assignment-list-container')]//td[contains(@class,'link')]")
      .then(function(flag){
            if(flag){
               return browser
                .waitForElementsByXPath("//div[contains(@class,'assignment-list-container')]//td[contains(@class,'link')]", asserters.isDisplayed, 60000)
                .then(function(ele){
                  assignmentCount = _.size(ele);
                  console.log(" Total number of existing assignment which is to be delete = "+assignmentCount);
                  takeQuizpo.deleteAssignmentsFromBrowser(browser, assignmentCount, done);
                });
            }else {
                return browser
                .refresh()
    ...
    deleteAssignmentFromBrowserOnListView: function (browser, astName, done) {
        commonutil.acceptAlerts(browser, true).then(function () {
            return  browser
                .waitForElementByXPath("//td[contains(text(),'" + astName + "')]")
                .click()
                .sleep(3000)
                .waitForElementByCss(".delete.ng-scope", asserters.isDisplayed, 60000)
                .click()
    ...
    verifyAssignmentNotPresent: function (browser, astName) {
        return  browser
            .hasElementByXPath("//td[contains(text(),'" + astName + "')]");
    },

    openAssignmentOnListView: function (browser, astName) {
        return  browser
            .waitForElementByXPath("//div[@class='assignment-list-container']//td[contains(text(),'" + astName + "')]", asserters.isDisplayed, 60000)
            .click();
    },
    ...
    retrieveSecondFilterType: function (browser) {
        return browser
            .waitForElementByXPath("(//div[@class='show-all-toggle ng-binding'])[1]", asserters.isDisplayed, 60000)
            .click()
            .waitForElementByXPath("(//div[contains(@class,'filter-column') and contains(.,'Type')]//ul//li//label)[2]", asserters.isDisplayed, 60000)
            .text();
    },
    ...
    retrieveSecondDifficultyType: function (browser) {
        return browser
            .waitForElementByXPath("(//div[@class='show-all-toggle ng-binding'])[3]", asserters.isDisplayed, 60000)
            .click()
            .waitForElementByXPath("(//div[contains(@class,'filter-column') and contains(.,'Difficulty')]//ul//li//label)[2]", asserters.isDisplayed, 60000)
            .text();
    },
    ...
    clickOnQuestionPerStudentTextBox: function (browser) {
        if(stringutil.removeBoundaryQuotes(process.env.RUN_IN_BROWSER.toString())==="safari"){
          return browser
              .execute("return document.getElementById('manual-select').click()")
          ...
          return browser
              .waitForElementByCss(".textRadio", asserters.isDisplayed, 60000).click()
              .waitForElementByCss("#ques-per-student", asserters.isDisplayed, 60000)
              .clear()
              .waitForElementByCss("#ques-per-student", asserters.isDisplayed, 60000)
              .type("4")
    ...
    if(stringutil.removeBoundaryQuotes(process.env.RUN_IN_BROWSER.toString())==="safari"){
      return browser
          .execute("return document.getElementById('select-type').click()");
    }else {
       return browser
    ...
    validateEditedAttempt: function (browser, astName, attempt) {
        return browser
            .waitForElementByXPath("//td[contains(text(),'"+astName+"')]//parent::tr//td[contains(@data-heading,'Attempts')]", asserters.isDisplayed, 60000)
            .text()
          ...
          .waitForElementByXPath("(//div[contains(@class,'datefield ng-binding')])["+index+"]", asserters.isDisplayed, 10000)
            ...
            return  browser.hasElementByXPath("(//div[@class='datepicker cg-calendar ng-isolate-scope']//div[@class='week ng-scope'])[last()]//div[@class='day ng-scope different-month selected']").then (function (currentDatePresenceStatus){
                if (currentDatePresenceStatus) {
                  return  browser
                      .waitForElementByCss("div[class='datepicker cg-calendar ng-isolate-scope'] .day.today", asserters.isDisplayed, 10000)
                      .click();
               }
                else {
                return  browser
    ...
    clickOnCalendar: function (browser) {
      return browser
          .waitForElementByXPath("//cg-date-picker//parent::section//cg-date-picker//div[@class='datefield ng-binding']", asserters.isDisplayed, 60000)
          .click();
    },
    ...
    doneButtonStatus: function (browser) {
      return browser
          .waitForElementByXPath("//cg-date-picker//parent::section//following-sibling::div//button[contains(@class,'confirm')]", asserters.isDisplayed, 60000).getAttribute('aria-disabled');
    },
    clickOnDoneButton: function (browser) {
      return browser
          .waitForElementByXPath("//cg-date-picker//parent::section//following-sibling::div//button[contains(@class,'confirm')]", asserters.isDisplayed, 60000)
          .click();
    },
...
getBackGrndColorOfBeforeDueDate: function(browser){
  return browser
  .sleep(1000)
  .execute("return getComputedStyle(document.querySelector('.cg-calendar.datepicker > div.week > div.day.before-today:not(.name):not(.selected)')).getPropertyValue('background-color')");
                ...
                .hasElementByXPath("(//div[contains(@class,'assignment-nav-wrapper')]//div[contains(@class,'week ng-scope')]//parent::div//div[@class='day ng-scope'])[1]").then(function (statusOfDaysRemainingInThisMonth) {
                  ...
                  if(statusOfDaysRemainingInThisMonth == true){
                      ...
                      dueDateOfNextMonth = 6-countsOfRemainingDaysInCurrentMonth;
                       ...
                       .click()
                       .sleep(1000)
                       .waitForElementByXPath("((//div[@class='span-half']//cg-date-picker[@label-text='Due Date']//div[@class='week ng-scope'])[1]//div//span[text()='"+dueDateOfNextMonth+"'])[last()]", asserters.isDisplayed, 60000)
                       .click();
                      }else{
                        return browser
                        .waitForElementByXPath("//div[@class='span-half']//cg-date-picker[@label-text='Due Date']//div[contains(@class,'next')]", asserters.isDisplayed, 10000)
                        .click()
                        .sleep(1000)
                        .waitForElementByXPath("((//div[@class='span-half']//cg-date-picker[@label-text='Due Date']//div[@class='week ng-scope'])[1]//div//span[text()='1'])[last()]", asserters.isDisplayed, 60000)
                        .click();
                      }
                    });
                  }else{
