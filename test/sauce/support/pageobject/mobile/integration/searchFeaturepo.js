...
module.exports = {
	...
	scrollToSearchResultsBottom: function(browser) {
		return browser
			.execute("return document.getElementsByClassName('copyright-labels ng-scope')[0].scrollIntoView(true)")
			.sleep(2000);
		// return browser
	...
	getSearchTextOnConceptTracker: function(browser) {
		return browser
			.sleep(1000)
			.execute("return (document.evaluate(\"(//div[contains(@class,'chart-container ng-scope')])[1]//div[contains(@class,'chartjs ng-scope')][1]//div[1]//h3[contains(@class,'chartjs-title ng-binding')]\", document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue).scrollIntoView()")
			.sleep(1000)
	...
	clickOnSearchTextOnConceptTracker: function(browser) {
		return browser
