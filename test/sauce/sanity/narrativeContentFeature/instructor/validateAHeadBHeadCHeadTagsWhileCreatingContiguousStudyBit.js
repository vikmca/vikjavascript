...
describe(scriptName + 'STUDYBIT:: VALIDATE A HEAD B HEAD C HEAD PUBLISHER TAGS FOR STUDYBITS', function() {
	...
	before(function(done) {
		...
		totalTime = 0;
		//Reports
		console.log(report.formatTestName("4LTR STUDYBIT:: , VALIDATE A HEAD B HEAD C HEAD PUBLISHER TAG AT INSTRUCTOR END"));
		console.log("======================================");
	...
	if (stringutil.removeBoundaryQuotes(process.env.RUN_IN_PLATFORM.toString()) == "mobile" || stringutil.removeBoundaryQuotes(process.env.RUN_IN_PLATFORM.toString()) === "iOS") {
		...
		it(". Delete all the created studybits in past if any", function(done) {
		...
		it(". Select text for B head", function(done) {
		...
		it(". Select text for C head ", function(done) {
		...
		it(". Validate increased tag count when user select text for C head", function(done) {
			studybit.fetchTheNumberOfPublisherTag(browser).then(function(element) {
