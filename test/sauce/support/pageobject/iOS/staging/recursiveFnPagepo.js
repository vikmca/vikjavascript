...
module.exports =  {
  ...
  validateLeftcountAtAstNameTextbox: function (browser, done) {
    assessmentsPage.fetchAssignmentNameTextBox(browser).getAttribute('value').then(function (assignmentTextName) {
        assignmentNameTextBoxText = assignmentTextName;
        assessmentsPage.characterCountAssignmentName(browser).then(function (length) {
            assignmentCharacterCount = length;
            function verifyCount() {
                if (assignmentCharacterCount <= initialLeftCountForAst) {
                    ...
                    assessmentsPage.fetchAssignmentNameTextBox(browser).clear().then(function () {
                        assessmentsPage.fetchAssignmentNameTextBox(browser).type(assignmentNameTextBoxText).then(function () {
  ...
  validateLeftcountAtNotesField: function (browser, done) {
    notespo.fetchValueFromNotesFieldTextBox(browser).getAttribute('value').then(function(textOnNotesField){
      textOfNotesField = textOnNotesField;
  ...
  verifyChaptersOnTopicPane: function (browser, done) {
    tocPage.navigateToSlidingToc(browser).text().then(function (text) {
    ...
    validateTopicsPresentOnChapterReading: function(browser, done){
     ...
     verifyEditedAstPresentOnCurrentDate: function(browser, done){
      tocPage.selectTileView(browser);
      return browser
      ...
      mediaquizpage.getQuestionsCount(browser).then(function (questionCount) {
        ...
        function mediaQuizQuestionsOnPreview() {
            if (countOfQuestion <= questionCountOnCue) {
                ...
                .sleep(2000)
                .waitForElementByXPath("//div[@class='cas-mediaquiz-question'][1]//div[@class='cas-task'][" + countOfQuestion + "]//div[@class='cas-choice'][1]//label", asserters.isDisplayed, 60000)
                 .then(function(quesLocation){
                  browser
                  .getLocationInView(quesLocation)
                  .sleep(1000)
                  .execute("window.scrollBy(0,-70)")
                  .sleep(1000)
                  // .waitForElementByXPath("//div[@class='cas-mediaquiz-question'][1]//div[@class='cas-task'][" + countOfQuestion + "]//div[@class='cas-choice'][1]//label", asserters.isDisplayed, 60000)
                  // .click();
                  .execute("return document.evaluate(\"//div[@class='cas-mediaquiz-question'][1]//div[@class='cas-task'][" + countOfQuestion + "]//div[@class='cas-choice'][1]//label\", document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue.click()");
                  countOfQuestion++;
                  mediaQuizQuestionsOnPreview();
                });
