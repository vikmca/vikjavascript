...
describe(scriptName + 'MEDIA QUIZ  ASSIGNMENT WITH DROP LOWEST SCORE TEST STRATEGY VALIDATION', function() {
	...
	before(function(done) {
		...
		console.log(report.formatTestScriptFileName("***validateEndToEndMediaQuiz.js***"));
	});
	...
	it(". Complete the Media Quiz Type assignment form Setting tab for system created assignment", function(done) {
		pageLoadingTime = 0;
	...
	it(". Save the Media Quiz Type assignment", function(done) {
		pageLoadingTime = 0;
	...
	it(". Launch the creaded Media Quiz Type assignment for first time", function(done) {
		pageLoadingTime = 0;
		if (assignmentCreationStatus === "failure") {
			this.timeout(courseHelper.getElevatedTimeout("timeoutForDependentFeature"));
		}
	...
	it(". LTR-5399 :: Verify error message should not be displayed on Media Quiz Type assignment launch and also validate Continue button should be appear on launched Media Quiz page", function(done) {
		pageLoadingTime = 0;
		if (assignmentCreationStatus === "failure") {
			this.timeout(courseHelper.getElevatedTimeout("timeoutForDependentFeature"));
		} else {
	...
	it(". LTR-4699 Validate CAS error message should not be display on launching the Media Quiz Type assignment at student end", function(done) {
	...
	it(". Verify the video is present at student end for Media Quiz Type assignment", function(done) {
		if (errorStatusAfterAssignmentLaunched) {
			console.log(report.reportHeader() +
			...
			mediaquizpage.validateIfPreviewVideoIsPresent(browser).then(function(videoPresenceStatus) {
				if (videoPresenceStatus) {
					console.log(report.reportHeader() +
						report.stepStatusWithData("[Media Quiz] : Video is presesnt at student end for Media Quiz Type assignment", videoPresenceStatus, "success") +
						report.reportFooter());
					done();
				} else {
					console.log(report.reportHeader() +
	...
	it(". Click on '?' button and verify presence of questions and log first question text for Media Quiz Type assignment", function(done) {
		if (errorStatusAfterAssignmentLaunched) {
			console.log(report.reportHeader() +
				report.stepStatusWithData("CAS Error message is displaying on launching the Media Quiz Type assignment at student end", "success") +
				report.reportFooter());
			this.skip();
		} else {
	...
	it(". Click on second question cue of Media Quiz Type assignment ", function(done) {
		if (errorStatusAfterAssignmentLaunched) {
			console.log(report.reportHeader() +
				report.stepStatusWithData("CAS Error message is displaying on launching the assignment at student end", "success") +
				report.reportFooter());
	...
	it(". Click on third question cue of Media Quiz Type assignment", function(done) {
		if (errorStatusAfterAssignmentLaunched) {
			console.log(report.reportHeader() +
				report.stepStatusWithData("CAS Error message is displaying on launching the assignment at student end", "success") +
				report.reportFooter());
	...
	it(". Validate the presence of result, fetch correct answers and total questions from the Media Quiz Type assignment's result page", function(done) {
		if (errorStatusAfterAssignmentLaunched) {
			console.log(report.reportHeader() +
				report.stepStatusWithData("CAS Error message is displaying on launching the assignment at student end", "success") +
				report.reportFooter());
	...
	it(". LTR-5329 :: Validate assignment(for Media Quiz) coulum should not disappear from the Student gradebook page ", function(done) {
	...
	it(". [Gradebook]Validate the Points earned by the student for MEdia Quiz type assignment", function(done) {
		this.retries(3);
		if (assignmentCreationStatus === "failure") {
			this.timeout(courseHelper.getElevatedTimeout("timeoutForDependentFeature"));
		}
	...
	it(". [Gradebook]Validate the Score(Questions Correct) should not be appeared on Student Gradebook table as this feature is obsolete", function(done) {
		if (assignmentCreationStatus === "failure") {
			this.timeout(courseHelper.getElevatedTimeout("timeoutForDependentFeature"));
		}
	...
	it(". Validate the presence of Class average value of Media Quiz Type assignment on student GradeBook", function(done) {
		if (assignmentCreationStatus === "failure") {
			this.timeout(courseHelper.getElevatedTimeout("timeoutForDependentFeature"));
		}
	...
	it(". Validate the presence of submission date of Media Quiz Type assignment on student Gradebook page", function(done) {
		if (assignmentCreationStatus === "failure") {
			this.timeout(courseHelper.getElevatedTimeout("timeoutForDependentFeature"));
		}
	...
	it(". Validate the presence of due date of Media Quiz Type assignment on student GradeBbook page", function(done) {
		if (assignmentCreationStatus === "failure") {
			this.timeout(courseHelper.getElevatedTimeout("timeoutForDependentFeature"));
		}
	...
	it(". Click on the current date cell for launching the Media quiz assignment", function(done) {
		if (assignmentCreationStatus === "failure") {
			this.timeout(courseHelper.getElevatedTimeout("timeoutForDependentFeature"));
		}
		studentAssessmentsPage.clickCurrentDateCell(browser, done);
	...
	it(". Launch the Media Quiz type assignment for taking 2nd attempt", function(done) {
		pageLoadingTime = 0;
		if (assignmentCreationStatus === "failure") {
			this.timeout(courseHelper.getElevatedTimeout("timeoutForDependentFeature"));
		}
	...
	it(". LTR-4699 Validate error message should not be display on launching the Media Quiz Type assignment at student end", function(done) {
		mediaquizpage.verifyErrorMessageOnPreview(browser).then(function(status) {
			errorStatusAfterAssignmentLaunched = status;
			if (errorStatusAfterAssignmentLaunched) {
				mediaquizpage.getErrorMessageOnPreview(browser).text().then(function(errorText) {
					console.log(report.reportHeader() +
						report.stepStatusWithData("CAS error message is displaying n launching the Media Quiz Type assignment at student end and text is " + errorText, "failure") +
						report.reportFooter());
				});
			} else {
				console.log(report.reportHeader() +
	...
	it(". Click on '?' button and verify presence of questions and log first question text of Media Quiz Type assignment", function(done) {
		if (errorStatusAfterAssignmentLaunched) {
			console.log(report.reportHeader() +
				report.stepStatusWithData("CAS Error message is displaying on launching the Media Quiz Type assignment at student end", "success") +
				report.reportFooter());
	...
	it(". Validate the presence of result, fetch correct answers and total questions from the quiz", function(done) {
		if (errorStatusAfterAssignmentLaunched) {
			...
			casPage.getQuestionsCorrect(browser).then(function(questionsCorrect) {
				console.log("Total Questions Correct " + questionsCorrect);
				questionsCorrectFromCAS2 = questionsCorrect;
				studentAssignmentCompletionStatus = "success";
				console.log(report.reportHeader() +
					report.stepStatusWithData("CAS : Student Completed the assignment  questions and got a score of  ", questionsCorrectFromCAS, "success") +
					report.reportFooter());
...
it(". Launch the of Media Quiz Type assignment for taking third attempt", function(done) {
	pageLoadingTime = 0;
	if (assignmentCreationStatus === "failure") {
		this.timeout(courseHelper.getElevatedTimeout("timeoutForDependentFeature"));
	}
...
it(". LTR-4699 Validate error message should not be display on launching the of Media Quiz Type assignment at student end", function(done) {
	mediaquizpage.verifyErrorMessageOnPreview(browser).then(function(status) {
		errorStatusAfterAssignmentLaunched = status;
		if (errorStatusAfterAssignmentLaunched) {
			mediaquizpage.getErrorMessageOnPreview(browser).text().then(function(errorText) {
...
it(". Verify the video is present for Media quiz assignmen at student end", function(done) {
	if (errorStatusAfterAssignmentLaunched) {
		console.log(report.reportHeader() +
			report.stepStatusWithData("CAS Error message is displaying on launching the assignment at student end", "success") +
			report.reportFooter());
...
it(". Validate the presence of result, fetch correct answers and total questions from the Media Quiz result page", function(done) {
	if (errorStatusAfterAssignmentLaunched) {
		console.log(report.reportHeader() +
		...
		casPage.getQuestionsCorrect(browser).then(function(questionsCorrect) {
			console.log("Total Questions Correct " + questionsCorrect);
			questionsCorrectFromCAS3 = questionsCorrect;
			studentAssignmentCompletionStatus = "success";
			console.log(report.reportHeader() +
				report.stepStatusWithData("CAS : Student Completed the assignment  questions and got a score of  ", questionsCorrectFromCAS, "success") +
				report.reportFooter());
...
it(". Get Average score value for Media Quiz type assignment ", function(done) {
	if (errorStatusAfterAssignmentLaunched) {
		console.log(report.reportHeader() +
			report.stepStatusWithData("CAS Error message is displaying on launching the assignment at student end", "success") +
			report.reportFooter());
		...
		var averageScoreInPoints = ((questionsCorrectFromCAS*totalPoints+questionsCorrectFromCAS2*totalPoints+questionsCorrectFromCAS3*totalPoints)*10)/(totalQuestionsInTheQuiz*3*10);
		console.log(averageScoreInPoints);
		averageScore = mathutil.getScoreUptoOneDecimal(averageScoreInPoints);
...
it(". LTR-5329 :: Validate Media Quiz type assignment's coulum should not disappear from the Student gradebook page ", function(done) {
	assessmentsPage.validateAssignmentDetailsOnGradebook(browser).then(function(astDetailsStatus) {
		if (astDetailsStatus) {
			console.log(report.reportHeader() +
				report.stepStatusWithData("Created Media Quiz type assignment details are appeared on student gradebook page", astDetailsStatus, "success") +
				report.reportFooter());
			done();
		} else {
			console.log(report.reportHeader() +
...
it(". [Gradebook]Validate the Points earned by the student for Media Quiz type assignment ", function(done) {
	...
	studenGradebookPage.getScoredPoints(browser, assessmentsPage.getAssignmentName()).then(function(valueScore) {
		if (valueScore.toString() === averageScore) {
			var pointsFromStudentGradebook = valueScore;
			console.log(report.reportHeader() + report.stepStatusWithData("GRADEBOOK : TestBot calculated point score " + averageScore + " points, is compared against the student point score retrieved from Student Gradebook ", valueScore, "success") + report.reportFooter());
			done();
		} else {
			console.log(report.reportHeader() + report.stepStatusWithData("GRADEBOOK : TestBot calculated point score " + averageScore + " points, is compared against the student point score retrieved from Student Gradebook ", valueScore, "failure") + report.reportFooter());
		}
	});
});

it(". [Gradebook]Validate the Score(Questions Correct) is not present from Student Gradebook table as its a obsolete feature", function(done) {
	if (assignmentCreationStatus === "failure") {
		this.timeout(courseHelper.getElevatedTimeout("timeoutForDependentFeature"));
	}
	studenGradebookPage.verifyStudentScoreColoumn(browser, assessmentsPage.getAssignmentName()).then(function(status) {
...
it(". Validate the presence of Class average value for Media Quiz type assignment on student GradeBook", function(done) {
	if (assignmentCreationStatus === "failure") {
		this.timeout(courseHelper.getElevatedTimeout("timeoutForDependentFeature"));
	}
	studenGradebookPage.validateAvgScoreOnStudentGradebookForMediaQuiz(browser, assessmentsPage.getAssignmentName(), averageScore, done);
});
...
it(". Validate the presence of due date of Media Quiz type assignment on student GradeBbook page", function(done) {
	...
	it(". Calcuate the total of score earned by students for Media Quiz type assignment  ", function(done) {
		if (assignmentCreationStatus === "failure") {
			this.timeout(courseHelper.getElevatedTimeout("timeoutForDependentFeature"));
		}
		var sizeOfColumn = 0;
	...
	it(". Calculate class average for a particular Media Quiz Type assignment ", function(done) {
		if (assignmentCreationStatus === "failure") {
			this.timeout(courseHelper.getElevatedTimeout("timeoutForDependentFeature"));
		}
		averageScoreForAllStudents = ((averageScore * 10 / parseInt(totalsudentcount) * 10) / 10) / 10;
		console.log(report.reportHeader() +
			report.stepStatusWithData("GRADEBOOK : Average class score for a particular Media Quiz Type assignment calculated and is coming out to be ::  " + averageScoreForAllStudents, "success") +
			report.reportFooter());
		done();
	});
	//LTR-5346 won't fix
	...
	it(". LTR-5346 Verify the presence of point gained by the student is reflected 0 on total points until of Media Quiz Type assignment due date has passed", function(done) {
		if (assignmentCreationStatus === "failure") {
			this.timeout(courseHelper.getElevatedTimeout("timeoutForDependentFeature"));
		}
	...
	it(". Verify the presence of point of Media Quiz Type assignment gained by the student", function(done) {
		if (assignmentCreationStatus === "failure") {
			this.timeout(courseHelper.getElevatedTimeout("timeoutForDependentFeature"));
		}
		instructorMainGradebookView.getScoreText(browser, 1)
			.then(function(pointsGainedByStudent) {
				...
				if (totalPointsGained.indexOf(averageScore) > -1) {
					console.log(report.reportHeader() +
						report.stepStatusWithData("GRADEBOOK : Total points earned by student " + totalPointsGained + ", is compared against the testbot calculated total points earned ", averageScore, "success") +
						report.reportFooter());
					done();
				} else {
					console.log(report.reportHeader() +
						report.stepStatusWithData("GRADEBOOK : Total points earned by student " + totalPointsGained + ", is compared against the testbot calculated total points earned ", averageScore, "failure") +
						report.reportFooter());
				}
			});
	});
	...
	it(". Validate presence of class average value for Media Quiz type assignment on student detailed page on instructor gradebook view ", function(done) {
	...
	it(". [Gradebook] Verify whether the system generated student point score is updated on Instructor Gradebook for Media Quiz type assignment on the Student Detailed Results Page", function(done) {
		...
		instructorGradebookStudentDetailedPage.getScoredPoints(browser, assessmentsPage.getAssignmentName())
			.then(function(scoredPoints) {
				if (scoredPoints.indexOf(averageScore) > -1) {
	...
	it(". Validate override score field should be disabled untill the due date has not passed", function(done) {
	...
	it(". Navigate to a specific student detailed view and edit attempt from the instructor gradebook for Media Quiz type assignment ", function(done) {
		if (assignmentCreationStatus === "failure" && assignmentNameClickableStatusAfterAttempt == false) {
			this.timeout(courseHelper.getElevatedTimeout("timeoutForDependentFeature"));
		}
		if (stringutil.removeBoundaryQuotes(process.env.RUN_IN_PLATFORM.toString()) === "iOS") {
	...
	it(". Validate attempts edited successfully without any error message for Media Quiz type assignment", function(done) {
	...
	it(". Re-validate edited attempts persist after refreshing the page without any error Media Quiz type assignment", function(done) {
		instructorGradebookStudentDetailedPage.editedAttemptOnStudentDetailPage(browser, assessmentsPage.getAssignmentName()).then(function(editedAttempt) {
			if (editedAttempt.indexOf("unlimited") > -1) {
				console.log(report.reportHeader() +
	...
	it(". Validate points possible to date is displayed correctly on the points graph for Media Quiz type assignment", function(done) {
		if (assignmentCreationStatus === "failure") {
			this.timeout(courseHelper.getElevatedTimeout("timeoutForDependentFeature"));
		}
	...
	it(". Validate the presence of the total points scored by the student on the points graph for Media Quiz type assignment", function(done) {
		if (assignmentCreationStatus === "failure") {
			this.timeout(courseHelper.getElevatedTimeout("timeoutForDependentFeature"));
		}
	...
	it(". Validate the presence of due date on student detailed page under instructor gradebook view for Media Quiz type assignment ", function(done) {
		if (assignmentCreationStatus === "failure") {
			this.timeout(courseHelper.getElevatedTimeout("timeoutForDependentFeature"));
		}
	...
	it(". Validate the presence of submission date on student detailed page under instructor gradebook view for Media Quiz type assignment", function(done) {
		...
		browser.refresh().then(function() {
			instructorGradebookStudentDetailedPage.checkSubmittedDate(browser, assessmentsPage.getAssignmentName(), dataUtil.getCurrentDateOnhyphenatedMMDDYYformat())
				.then(function() {
					console.log(report.reportHeader() +
						report.stepStatusWithData("GRADEBOOK : Submission date of Media Quiz " + dataUtil.getCurrentDateOnhyphenatedMMDDYYformat() + "  is compared against current date of assignment submission", dataUtil.getCurrentDateOnhyphenatedMMDDYYformat(), "success") +
						report.reportFooter());
					done();
				});
		});
	...
	it(". Click on back button to navigate to main gradebook view and click on the attempted Media Quiz link", function(done) {
	...
	it(". LTR-4665 :: Validate page should appear without any error", function(done) {
		...
		instructorMainGradebookView.getErrorMessagePresenceStatus(browser).then(function(errorMessagePresencestatus) {
			if (!errorMessagePresencestatus) {
				console.log(report.reportHeader() +
					report.stepStatusWithData("GRADEBOOK :: Media Quiz detailed page is appearing without any error", errorMessagePresencestatus, "success") +
					report.reportFooter());
				done();
			} else {
				console.log(report.reportHeader() +
					report.stepStatusWithData("GRADEBOOK :: Media Quiz detailed page is appearing without any error", errorMessagePresencestatus, "failure") +
					report.reportFooter());
			}
		});
	});

	it(". LTR-4665 :: Validate Media Quiz name present on assignment detailed page", function(done) {
		if (assignmentCreationStatus === "failure") {
			this.timeout(courseHelper.getElevatedTimeout("timeoutForDependentFeature"));
		}
		instructorAssessmentDetailedInfopo.getAssessmentNameOnStudentAssessmentDetailedView(browser).then(function(assignmentname) {
			if (assignmentname.indexOf(assessmentsPage.getAssignmentName()) > -1) {
				console.log(report.reportHeader() +
					report.stepStatusWithData("GRADEBOOK : Media Quiz name " + assignmentname + "  is compared against assessment name", assessmentsPage.getAssignmentName(), "success") +
					report.reportFooter());
				done();
			} else {
				console.log(report.reportHeader() +
					report.stepStatusWithData("GRADEBOOK : Media Quiz name " + assignmentname + "  is compared against assessment name", assessmentsPage.getAssignmentName(), "failure") +
					report.reportFooter());
			}
		});
	});
	...
	it(". LTR-4665 :: Validate the presence of score expected on the submission graph for Media Quiz type assignment", function(done) {
		if (assignmentCreationStatus === "failure") {
			this.timeout(courseHelper.getElevatedTimeout("timeoutForDependentFeature"));
		}
	...
	it(". LTR-4665 :: Validate the presence of Scores, Distribution, Submissions headers on assignment detailed view for Media Quiz type assignment", function(done) {
		if (assignmentCreationStatus === "failure") {
			this.timeout(courseHelper.getElevatedTimeout("timeoutForDependentFeature"));
		}
	...
	it(". Navigate on main Gradebook page", function(done) {
		if (assignmentCreationStatus === "failure") {
			this.timeout(courseHelper.getElevatedTimeout("timeoutForDependentFeature"));
		}
		menuPage.selectGradebook(userType, browser, done);
	...
	it(". Navigate to details of student's attempts(Media Quiz Result page) view", function(done) {
		if (errorStatusAfterAssignmentLaunched) {
			console.log(report.reportHeader() +
				report.stepStatusWithData("CAS Error message is displaying on launching the assignment at student end", "success") +
				report.reportFooter());
	...
	it(". LTR-5439 :: Validates all analytics for Correct question count and total question count on instructor's Media Quiz Result page", function(done) {
		this.retries(3);
		...
		instructorGradebookNavigationPage.getAnalyticsValues(browser, "#score", PercentageValueUptoOneDecimalPoint).then(function() {
			...
			instructorGradebookNavigationPage.getAnalyticsValues(browser, "#correct", questionsCorrectFromCAS).then(function() {
				...
				instructorGradebookNavigationPage.getAnalyticsValues(browser, "#total", totalQuestionsInTheQuiz).then(function() {
					console.log(totalQuestionsInTheQuiz);
					console.log(report.reportHeader() +
						report.stepStatusWithData("All analytics are present on Media Quiz Result page",
