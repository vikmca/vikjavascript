...
describe(scriptName + 'STUDENT :: NOTES CREATION AND STUDYBOARD VALIDATION', function() {
	var browser;
	var allPassed = true;
	var userType;
	var courseName;
	...
	before(function(done) {
		...
		totalTime = 0;
		//Reports
		console.log(report.formatTestName("STUDENT :: NOTES CREATION AND VALIDATION"));
	...
	it(". Fetch existing StudyBits metrics on a chapter if any ", function(done) {
		conceptrackerPageInstructor.isConceptTrackerLoaded(browser).text().should.become('Concept Tracker').then(function() {
			browser.execute("return document.evaluate(\"count(//div[contains(@class,'chart-container ng-scope')]/h1[contains(.,'" + productData.concepttracker.quiz.chapterbased.desiredChapterForStudybit + "')])\", document, null, XPathResult. NUMBER_TYPE, null ).numberValue").then(function(status) {
				if (status == 1) {
					browser.waitForElementByXPath("//h1[contains(.,'" + productData.concepttracker.quiz.chapterbased.desiredChapterForStudybit + "')]/following-sibling::div[contains(@class,'chartjs overview')]//div[@class='studybits']//div[contains(@class,'studybit-count')]", asserters.isDisplayed, 90000).text().then(function(studybitCountBeforeEdit) {
						studybitCount = parseInt(studybitCountBeforeEdit) + 1;
						console.log(report.reportHeader() +
							report.stepStatusWithData("ANALYTICS :: StudyBits present for \" " + productData.concepttracker.quiz.chapterbased.desiredChapterForStudybit + " :", studybitCountBeforeEdit, "success") +
							report.reportFooter());
						done();
					});
				} else {
					console.log("Chapter not present on Concept tracker");
					studybitCountBeforeEdit = 0;
					console.log(report.reportHeader() +
						report.stepStatusWithData("ANALYTICS :: StudyBits present for \" " + productData.concepttracker.quiz.chapterbased.desiredChapterForStudybit + " :", studybitCountBeforeEdit, "success") +
						report.reportFooter());
					done();
	...
	it(". Validate Notes created by student should not reflect as Studybit on Instructor concept tracker if they are edited on StudyBoard view", function(done) {
		browser.waitForElementByXPath("//h1[contains(.,'" + productData.concepttracker.notesBased.desiredChapterForStudybit + "')]/following-sibling::div[contains(@class,'chartjs overview')]//div[@class='studybits']//div[contains(@class,'studybit-count')]", asserters.isDisplayed, 90000).text().then(function(studybitCountAfterEditNotes) {
			var studybitCountOnConceptTracker = stringutil.returnValueAfterSplit(studybitCountAfterEditNotes, " ", 0);
			console.log(report.reportHeader() +
				report.stepStatusWithData("ANALYTICS :: StudyBits present for \" " + productData.concepttracker.notesBased.desiredChapterForStudybit + " :", studybitCountOnConceptTracker, "success") +
				report.reportFooter());
			if (studybitCount === parseInt(studybitCountOnConceptTracker)) {
				console.log(report.reportHeader() +
					report.stepStatusWithData("ANALYTICS :: Studybits present for \" " + productData.concepttracker.notesBased.desiredChapterForStudybit + " :", studybitCount, "failure") +
					report.reportFooter());
				done();
			} else {
				console.log(report.reportHeader() +
					report.stepStatusWithData("ANALYTICS :: StudyBits present for \" " + productData.concepttracker.notesBased.desiredChapterForStudybit + " :", studybitCount, "success") +
					report.reportFooter());
				done();
			}
		});
