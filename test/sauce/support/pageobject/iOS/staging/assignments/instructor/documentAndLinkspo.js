...
clickOnCreatedAssignment: function (browser) {
    ...
    .refresh()
    .sleep(6000)
    .waitForElementByCss(".event", asserters.isDisplayed, 60000)
    .click()
    .nodeify(done);
  },
...
return browser
     .waitForElementByXPath("//div[@class='full-width attachments']//li[contains(text(),"+loginPage.getProductData().chapter.topic.documents.assignments[0].documents[0].name+")]", asserters.isDisplayed, 60000).then(function(){
       return browser
       .waitForElementByXPath("//div[@class='full-width attachments']//li[contains(text(),"+loginPage.getProductData().chapter.topic.documents.assignments[0].documents[1].name+")]", asserters.isDisplayed, 60000).then(function(){
       done();
     });
     });
}
