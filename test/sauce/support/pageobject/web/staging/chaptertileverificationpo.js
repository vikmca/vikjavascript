...
module.exports = {
    ...
    return browser
      .waitForElementByCss(".flashcard-review.chapter-flashcard.ng-scope header h6", asserters.isDisplayed, 10000).text().should.eventually.include("Concepts Review");

    },

    verifyFlashcardTotalCounterLabel: function (browser) {
    console.log("productData.chapter.topic.flashcard.flashcardontoc.totalcards  "+productData.chapter.topic.flashcard.flashcardontoc.totalcards);
    return browser
    ...
    verifyPresenceOfShowMoreLabel: function (browser) {
    return browser
      .waitForElementByCss(".show-more.ng-scope", asserters.isDisplayed, 10000).text().should.eventually.include("see all flashcards");
    ...
    verifycarouselview: function (browser, done, idofheading, heading, idofsecondimage) {
        browser
          .waitForElementsByXPath("//li[@class='banner ng-scope']//ul[@class='featured-items']/li", asserters.isDisplayed, 10000).then(function(feature){
            if (_.size(feature) == 6) {
                console.log(report.reportHeader() +
                    report.stepStatusWithData("Verify the presence of 6 Chapter highlight boxes on Carousel View", "success") +
                    report.reportFooter());
                 browser
                    .waitForElementByXPath("(//ul[@class='featured-items']//li)[1]/a/div/img", 3000).getAttribute('src').then(function (href) {
    ...
    validateTotalChapterPresentOnChapterTilesView: function (browser){
      return browser
          .waitForElementsByCss(".chapter", asserters.isDisplayed, 30000);
    },
    getChapterViewedCount: function (browser){
      return browser
          .waitForElementsByXPath("//p[contains(@class,'chapter-timestamp-text') and contains(text(),'Not Viewed')]", asserters.isDisplayed, 30000);
    },
    chapterViewedMessageAppear: function (browser){
      return browser
        .waitForElementByXPath("//p[contains(@class,'chapter-timestamp-text') and contains(text(),'less than an hour ago')]", asserters.isDisplayed, 30000);
    },
                ...
                if (attributeicon.indexOf("video-featured-icon") > -1) {
                  ...
                  .then(function(element){
                    ...
                    .then(function(property){
                      browser
                      .waitForElementByXPath("(//li[@class='banner ng-scope']//ul[@class='featured-items']//li)[" + countcarousel + "]//img").getAttribute('alt').then(function(ele){
                        if (ele === null) {
  ...
  createNewCoursepo.closePopupWindow(browser);
  return browser
      .waitForElementsByXPath("//li[@class='banner ng-scope']//ul[@class='featured-items']/li", asserters.isDisplayed, 10000).then(function(feature){
        if (_.size(feature) == 6) {
  ...
  verifydescriptionBelowConceptsReview: function(browser) {
    return browser
      .waitForElementByCss(".flashcard-review-description.ng-scope", asserters.isDisplayed, 10000).text().should.eventually.include("Preview the core concepts you will read about in this chapter.");
  },
