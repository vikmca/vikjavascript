...
describe(scriptName + 'READER SPEAKER VALIDATION', function() {
	...
	before(function(done) {
		...
		console.log(report.formatTestScriptFileName("***instructor/validateReaderSpeaker.js***"));
	});
	...
	it(". LTR-5453 :: Validate that course should be launched successfully and should not be navigated to login page", function(done) {
		...
		it(". Validate Progress bar and volume button should be present", function(done) {
		...
		it(". Validate that setting button should be present on expanded view and settting button window should not be displayed without clicking on setting icon", function(done) {
		...
		it(". Click on settting button and validate setting window should be displayed", function(done) {
		...
		it(". Validate Reader Speaker Logo on reader speaker setting window ", function(done) {
		...
		it(". Validate that reader speaker listen should present on reader speaker setting window ", function(done) {
		...
		it(". Validate that reader speaker heading text should present on reader speaker setting window ", function(done) {
		...
		it(". Validate that Speed, popup and restore menu should present on reader speaker setting window ", function(done) {
		...
		it(". Validate that Close button should present on reader speaker setting window", function(done) {
		...
		it(". Validate that cross button should be present on reader speaker setting window for closing the popup button", function(done) {
		...
		it(". Validate that the labels should present under Reading panel on reader speaker setting window", function(done) {
					...
					readerSpeakerpo.verifyLabelsOnReadingPanel(browser, 3, testData.readerSpeaker.readinglabel3).then(function() {
						console.log(report.reportHeader() +
							report.stepStatusWithData("Labels ",
								...
								" present under Reading panel on reader speaker setting window", "success") +
							report.reportFooter());
						done();
					});
				});
		...
		it(". Validate the labels under General panel on reader speaker setting window", function(done) {
		...
		it(". Validate that the buttons and labels should present under Restore default setting panel on reader speaker setting window", function(done) {
			readerSpeakerpo.verifyLabelsOnRestoreSettingPanel(browser, testData.readerSpeaker.restorelabel1).then(function() {
				readerSpeakerpo.verifyButtonOnRestoreSettingPanel(browser).getAttribute('value').then(function(buttonText) {
					if (buttonText.indexOf(testData.readerSpeaker.restoreButtonText) > -1) {
						console.log(report.reportHeader() +
							report.stepStatusWithData("Reader Speaker Restore button's text " + testData.readerSpeaker.restoreButtonText + " is compared against ", buttonText, "success") +
							report.reportFooter());
						done();
					} else {
						console.log(report.reportHeader() +
							report.stepStatusWithData("Reader Speaker Restore button's text " + testData.readerSpeaker.restoreButtonText + " is compared against ", buttonText, "failure") +
							report.reportFooter());
					}
				});
			});
		...
		it(". Validate checkbox state should be changed after clicking on slow label checkbox under setting window ", function(done) {
		...
		it(". Validate on clicking restore default setting button then default checkbox should be selected on setting window should be closed", function(done) {
		...
		it(". Validate on clicking on close button setting window should be closed", function(done) {
			readerSpeakerpo.clickOnCloseButton(browser).then(function() {
				readerSpeakerpo.settingWindowDisplayedStatus(browser).then(function(settingWindowDisplayedNoneStatus) {
		...
		it(". Validate on reader speaker tray should be in expanded view after closing the setting window", function(done) {
			readerSpeakerpo.validateReaderSpeakerTrayExpanded(browser).then(function(expandedReaderSpeakerStatus) {
				if (expandedReaderSpeakerStatus) {
					console.log(report.reportHeader() +
						report.stepStatusWithData("Reader Speaker tray expanded", expandedReaderSpeakerStatus, "success") +
		...
		it(". Validate that Close button should prsent on Reader Speaker tray", function(done) {
