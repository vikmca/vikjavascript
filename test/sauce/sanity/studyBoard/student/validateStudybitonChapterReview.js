...
describe(scriptName + 'STUDENT :: VALIDATE USER IS ABLE TO CREATE STUDYBIT ON CHAPTER REVIEW', function() {
	var browser;
	var allPassed = true;
	var userType;
	var courseName;
	...
	before(function(done) {
		...
		totalTime = 0;
		//Reports
		console.log(report.formatTestName("STUDENT :: 4LTR FEATURES :: USER IS ABLE TO CREATE STUDYBIT ON CHAPTER REVIEW PAGE"));
		console.log("======================================");
	...
	it(". Fetch existing StudyBits metrics on a chapter if any ", function(done) {
		conceptrackerPageInstructor.isConceptTrackerLoaded(browser).text().should.become('Concept Tracker').then(function() {
			browser.execute("return document.evaluate(\"count(//div[contains(@class,'chart-container ng-scope')]/h1[contains(.,'" + productData.chapter.topic.studybit.keytermOnChapterReview.name + "')])\", document, null, XPathResult. NUMBER_TYPE, null ).numberValue").then(function(status) {
				if (status == 1) {
					// browser.waitForElementByXPath("//h1[contains(.,'" + productData.concepttracker.quiz.studybitbased.name + "')]/following-sibling::div[contains(@class,'chartjs overview')]//div[@class='studybits']//div[contains(@class,'studybit-count')]", asserters.isDisplayed, 90000).text()
					studybit.getStudyBitCountTextOnCT(browser, productData.chapter.topic.studybit.keytermOnChapterReview.name)
						.then(function(studybitCount) {
							console.log(report.reportHeader() +
								report.stepStatusWithData("ANALYTICS :: StudyBits present for \" " + productData.chapter.topic.studybit.keytermOnChapterReview.name + " :", studybitCount, "success") +
								report.reportFooter());
							done();
						});
				} else {
	...
	it(". Validate the count of StudyBits on instructor ConceptTracker after student's quiz attempts ", function(done) {
		browser.sleep(3000).execute("return document.evaluate(\"count(//div[contains(@class,'chart-container ng-scope')]/h1[contains(.,'" + productData.chapter.topic.studybit.keytermOnChapterReview.name + "')])\", document, null, XPathResult. NUMBER_TYPE, null ).numberValue").then(function(status) {
			console.log("status" + status);
			if (status == 1) {
				studybit.getStudyBitCountTextOnCT(browser, productData.chapter.topic.studybit.keytermOnChapterReview.name).then(function(StudyBitCounts) {
					var StudyBitCountsOnInstructorCT = parseInt(stringutil.returnValueAfterSplit(StudyBitCounts, " ", 0));
					if (StudyBitCountsOnInstructorCT) {
						console.log(report.reportHeader() +
							report.stepStatusWithData("Instructor ConceptTracker :: StudyBit Count is reflected under on ConceptTracket", StudyBitCountsOnInstructorCT, "success") +
							report.reportFooter());
						done();
					} else {
						console.log(report.reportHeader() +
							report.stepStatusWithData("Instructor ConceptTracker :: StudyBit Count is reflected under on ConceptTracket", StudyBitCountsOnInstructorCT, "failure") +
							report.reportFooter());
					}
				});
			} else {
	...
	it(". Validate unassigned concept is present on ConceptTracker", function(done) {
		if(product === "MKTG9"){
		studybit.validateUnassignedTagPresenceStatus(browser, productData.chapter.topic.studybit.keytermOnChapterReview.name, testData.ConceptTracker.tagValue).then(function(unassignedTagPresenceStatus) {
			if (unassignedTagPresenceStatus) {
				console.log(report.reportHeader() +
					report.stepStatusWithData("Instructor ConceptTracker :: Unassigned tag is present on ConceptTracket", unassignedTagPresenceStatus, "success") +
					report.reportFooter());
				done();
			} else {
				console.log(report.reportHeader() +
					report.stepStatusWithData("Instructor ConceptTracker :: Unassigned tag is present on ConceptTracket", unassignedTagPresenceStatus, "failure") +
					report.reportFooter());
			}
		});
		}else{
	...
	it(". Validate Studybit count is reflected under unassigned concept", function(done) {
		if(product === "MKTG9"){
		studybit.getStudyBitCountsUnderUnassigned(browser, productData.chapter.topic.studybit.keytermOnChapterReview.name, testData.ConceptTracker.tagValue).then(function(unassignedStudyBitCount) {
			sbCountOnUnassignedConcept = parseInt(stringutil.returnValueAfterSplit(unassignedStudyBitCount, " ", 0));
			if (sbCountOnUnassignedConcept) {
				console.log(report.reportHeader() +
					report.stepStatusWithData("Instructor ConceptTracker :: StudyBit Count is reflected under unassigned Concept present on ConceptTracket", unassignedStudyBitCount, "success") +
					report.reportFooter());
				done();
			} else {
				console.log(report.reportHeader() +
					report.stepStatusWithData("Instructor ConceptTracker :: StudyBit Count is reflected under unassigned Concept present on ConceptTracket", unassignedStudyBitCount, "failure") +
					report.reportFooter());
			}
		});
		}else{
