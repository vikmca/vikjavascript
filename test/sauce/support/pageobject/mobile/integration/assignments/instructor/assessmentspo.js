...
module.exports = {
    ...
    deleteAssignment: function (browser, done) {
     ...
     .sleep(1000)
     .hasElementByXPath("//div[@class='ui-grid-canvas']//div[@class='ui-grid-row ng-scope']")
     .then(function(flag){
           if(flag){
              return browser
               .waitForElementsByXPath("//div[@class='ui-grid-canvas']//div[@class='ui-grid-row ng-scope']", asserters.isDisplayed, 60000)
               .then(function(ele){
    ...
    clickOnQuestionPerStudentTextBox: function(browser) {
        ...
        .waitForElementByCss(".textRadio", asserters.isDisplayed, 60000)
        .click();
        // .sleep(1000)
        ...
        // .waitForElementByCss("#ques-per-student", asserters.isDisplayed, 60000)
        // .type("4")
        // .execute("return document.getElementById('ques-per-student').getAttribute('aria-disabled')")
        // .waitForElementByCss("#ques-per-student", asserters.isDisplayed, 60000);
    },
    clickOnAllSelectedBelowRadiButton: function (browser) {
        return browser
          // .hideKeyboard()
          .execute("return document.getElementsByClassName('nudge-right-first ng-isolate-scope')[0].getElementsByTagName('input')[0].click()");
...
clickOnAssessmentCreatedOnNextWeek: function (browser, day) {
    ...
    var resFinal =stringutil.returnreplacedstring(res, "[[]]", day);
    return browser
        .execute("return (document.evaluate(\""+resFinal+"\", document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue).scrollIntoView()")
        .sleep(1000)
        .execute("return window.scrollBy(0,-100)")
