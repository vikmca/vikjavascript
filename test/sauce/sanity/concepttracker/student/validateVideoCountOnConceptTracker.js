...
describe(scriptName + '4LTR (' + 'Instructor/Student' + ') :: VIDEO VIEWED BY STUDENT ON INSTRUCTOR CONCEPT TRACKER VALIDATION', function() {
	...
	before(function(done) {
		...
		totalTime = 0;
		//Reports
		console.log(report.formatTestName("ANALYTICS ::  VIDEO VIEWED BY STUDENT ON INSTRUCTOR CONCEPT TRACKER VALIDATION"));
	...
	if (process.env.RUN_IN_PLATFORM.toString() === "\"iOS\"") {
		...
		if (stringutil.removeBoundaryQuotes(process.env.RUN_FOR_PRODUCT.toString()) == "MIS7") {
			...
			it(". LTR-5482 :: Validate download video button should be displayed below the video", function(done) {
								...
								videopo.getTextOfDownloadVideoButton(browser).text()
									.then(function(downloadVideoBtnText) {
										if (downloadVideoBtnText.indexOf(testData.video.downloadVideo) > -1) {
											...
											console.log(report.reportHeader() +
												report.stepStatusWithData("LTR-5482 :: Download video button text " + downloadVideoBtnText + " is compared with", testData.video.downloadVideo, "failure") +
												report.reportFooter());
										}
									});
							} else {
								console.log(report.reportHeader() +
									report.stepStatusWithData("LTR-5482 :: Download Video button is present", downloadVideoBtnStatus, "failure") +
									report.reportFooter());
							}
						});
				}
			...
			it(". LTR-5482 :: Click on transcript button and validate two transcript buttons should be displayed", function(done) {
				if (stringutil.removeBoundaryQuotes(process.env.RUN_FOR_PRODUCT.toString()) == "PSYCH4") {
					this.skip();
				} else {
						...
						videopo.verifyTopHideTrancriptBtn(browser).then(function(topHideTrancriptStatus) {
							videopo.verifyBottomHideTrancriptBtn(browser).then(function(bottomHideTrancriptStatus) {
								if (topHideTrancriptStatus && bottomHideTrancriptStatus) {
										...
										.then(function(topHideTrancriptText) {
											videopo.getTextBottomHideTrancriptBtn(browser).text().then(function(bottomHideTrancriptText) {
												if ((topHideTrancriptText.indexOf(testData.video.hidetranscript) > -1) && (bottomHideTrancriptText.indexOf(testData.video.hidetranscript) > -1)) {
													...
													console.log(report.reportHeader() +
														report.stepStatusWithData("LTR-5482 :: Top Hide Transcript button text " + topHideTrancriptText + " and bottom transcript button text " + bottomHideTrancriptText + " compared with", testData.video.hidetranscript, "failure") +
														report.reportFooter());
												}
											});
										});
									...
									console.log(report.reportHeader() +
										report.stepStatusWithData("LTR-5482:: Top Hide Transcript button is present " + topHideTrancriptStatus + " and bottom hide transcript button is present", bottomHideTrancriptStatus, "failure") +
										report.reportFooter());
								}
							});
						});
			...
			it(". LTR-5482 :: Validate transcript should be displayed when user click on show transcript button", function(done) {
				if (stringutil.removeBoundaryQuotes(process.env.RUN_FOR_PRODUCT.toString()) == "PSYCH4") {
					this.skip();
				} else {
					videopo.validateTrancript(browser).then(function(transcriptVisibilityStatus) {
						if (transcriptVisibilityStatus) {
							...
							console.log(report.reportHeader() +
								report.stepStatusWithData("LTR-5482 ::Transcript is displaying on clicking on Show Transcript Button", transcriptVisibilityStatus, "failure") +
								report.reportFooter());
						}
					});
				}
			...
			it(".  LTR-5482 :: Validate download video button should be displayed below the video after clicking on show transcript button", function(done) {
				if (stringutil.removeBoundaryQuotes(process.env.RUN_FOR_PRODUCT.toString()) == "PSYCH4") {
					this.skip();
				} else {
					videopo.verifyDownloadVideoButtonIsDisplayed(browser)
						.then(function(downloadVideoBtnStatus) {
							if (downloadVideoBtnStatus) {
								...
								console.log(report.reportHeader() +
									report.stepStatusWithData("LTR-5482::Download Video button is still present after clicking on show transcript", downloadVideoBtnStatus, "failure") +
									report.reportFooter());
							}
						});
				}
			...
			it(".  LTR-5482:: Click on HIDE TRANSCRIPT button and validate transcript should not be displayed", function(done) {
				if (stringutil.removeBoundaryQuotes(process.env.RUN_FOR_PRODUCT.toString()) == "PSYCH4") {
					this.skip();
				} else {
					videopo.clickOnOnTranscriptButton(browser).then(function() {
						videopo.validateTrancriptHideOnNarrtive(browser).then(function(HideTrancriptStatus) {
							if (HideTrancriptStatus) {
								...
								console.log(report.reportHeader() +
									report.stepStatusWithData("LTR-5482::Transcript is displaing on clicking on Show Transcript Button", transcriptVisibilityStatus, "failure") +
									report.reportFooter());
							}
						});
					});
			...
			it(". Verify the count of video viewed after video play and pause with narrative content of registered user", function(done) {
				conceptrackerPageInstructor.getValueVideoViewed(browser, productData.concepttracker.video.chapter, 1).then(function(viewedlabel1) {
					preExistingViewed = viewedlabel1;
					conceptrackerPageInstructor.getValueVideoViewed(browser, productData.concepttracker.video.chapter, 2).then(function(viewedlabel2) {
						preExistingStudentsViewedAtLeastOne = viewedlabel2;
						conceptrackerPageInstructor.getValueVideoViewed(browser, productData.concepttracker.video.chapter, 3).then(function(viewedlabel3) {
							...
							if (parseInt(viewedlabel3) === preExistingTotalView) {
								console.log(report.reportHeader() +
									report.stepStatusWithData("Recorded count of Total Views " + preExistingTotalViews + " is increased by 1 after play video within narrative content of student", preExistingTotalView, "success") +
									report.reportFooter());
								done();
							} else {
								console.log(report.reportHeader() +
									report.stepStatusWithData("Recorded count of Total Views " + preExistingTotalViews + " is not increased by ", 1, "failure") +
									report.reportFooter());
							}
						});
					});
