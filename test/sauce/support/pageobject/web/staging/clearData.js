...
module.exports = {
    ...
    clearStudyboard: function (browser, done) {
     ...
     clearStudyboardWithoutPageRefresh: function (browser, done) {
         commonutil.acceptAlerts(browser,true).then(function(){
             return  browser
               .execute("for(i=0;i<(document.getElementsByClassName('icon-trash-gray').length);i++){document.getElementsByClassName('icon-trash-gray')[i].click();window.scrollBy(0,5)}").then(function(){
               return  browser
        ...
        return  browser
            ...
            .execute("for(i=0;i<(document.getElementsByClassName('icon-trash-gray').length);i++){document.getElementsByClassName('icon-trash-gray')[i].click();window.scrollBy(0,5)}").then(function(){
              if(process.env.RUN_IN_BROWSER.toString() === "\"internet explorer\""){
                ...
                return  browser
                    .refresh()
                    .waitForElementsByCss("header *", asserters.isDisplayed, 50000)
                    .sleep(3000)
                    .waitForElementByCss("ul li[class='ng-isolate-scope active']", asserters.isDisplayed, 90000)
                    .nodeify(done);
                  }
    ...
    clearCreatedCourse : function (browser, done, courseName) {
        commonutil.acceptAlerts(browser,true).then(function(){
            return  browser
            .waitForElementByXPath("//td[contains(.,'" + courseName + "')]/following-sibling::td/a[contains(@title,'Delete Course')]", asserters.isDisplayed, 10000)
            ...
            .hasElementByXPath("//td[contains(.,'" + courseName + "')]/following-sibling::td/a[contains(@title,'Delete Course')]",60000).then(function(duplicateCopyCourseStatus){
              ...
              if(duplicateCopyCourseStatus){
                return browser
                    ...
                    .click().then(function(){
                    // .execute("return document.evaluate(\"//td[contains(.,'" + courseName + "')]/following-sibling::td/a[contains(@title,'Delete Course')]\", document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue.click()").then(function(){
                    done();
                  });
              }else {
    ...
    hedeCreatedCourse: function (browser, done, courseName) {
        commonutil.acceptAlerts(browser,true).then(function(){
            return  browser
            .hasElementByXPath("//td[contains(.,'" + courseName + "')]/following-sibling::td/a[contains(@title,'hide')]",60000).then(function(hideCourse){
              console.log("duplicateCopyCourseStatus"+hideCourse);
              if(hideCourse){
                return browser
