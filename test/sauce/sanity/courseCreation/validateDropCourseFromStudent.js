...
describe(scriptName + 'DROP COURSE FROM STUDENT', function() {
	var browser;
	var allPassed = true;
	var userType;
	var setDate;
	...
	before(function(done) {
		...
		editedCourseName = product + " " + "EDITED COURSE NEW STUDENT";
		//Reports
		console.log(report.formatTestName("CENGAGE COURSE CREATION AND NEW STUDENT REGISTRATION AND VALIDATE DROP COURSE FROM STUDENT"));
		console.log(report.formatTestData(data.urlForLogin, data.userId, product, courseName));
		console.log(report.formatTestScriptFileName("***validateDropCourseFromStudent.js***"));
	});
	...
	it(". Navigate on GradeBook page and drop the student if any duplicate course is present", function(done) {
		this.timeout(courseHelper.getElevatedTimeout("additionalLoadTimeFeature"));
		if (editedCourseNameStatus) {
			basicpo.checkEula(browser).then(function(eula) {
				console.log(report.reportHeader() +
	...
	it(". Fill the section ", function(done) {
	...
	if (courseAggregationStatus !== "success") {
		...
		it(". Validate Instructor is logged out when user clicks on 'Cancel' button under Eula", function(done) {
			dropCourseFromStudent.checkIfStudentLoggedOut(browser).text().then(function(newStudentUserText) {
				if (newStudentUserText === testData.DropCourseStudent.dropCourseFromStudent.dashboardTextIfNoCoursePresent || newStudentUserText === testData.DropCourseStudent.dropCourseFromStudent.intDashboardTextIfNoCoursePresent) {
					console.log(report.reportHeader() +
		...
		it(". Validate created course name is present on student SSO dashboard", function(done) {
			loginPage.validateCourseName("student", courseName, browser).then(function(courseNameOnStudentStatus) {
				if (courseNameOnStudentStatus) {
					console.log(report.reportHeader() +
						report.stepStatusWithData("Created course is present on student SSO dashboard", courseNameOnStudentStatus, "success") +
						report.reportFooter());
		...
		it(". Validate edited course name should not displayed as old name", function(done) {
			if(stringutil.removeBoundaryQuotes(process.env.RUN_FOR_REGION.toString()) == "ASI"){
				this.skip();
			}else{
			loginPage.validateCourseName(userType, courseName, browser).then(function(oldCourseNameOnStudentStatus) {
				if (oldCourseNameOnStudentStatus) {
					console.log(report.reportHeader() +
						report.stepStatusWithData("Old course name is present on student SSO dashboard after editing", oldCourseNameOnStudentStatus, "failure") +
						report.reportFooter());
						userType = "studentforDropCourse";
						data = loginPage.setLoginData(userType);
						console.log(report.printLoginDetails(data.userId, data.password));
					...
					console.log(report.reportHeader() +
						report.stepStatusWithData("Old course name is not present on student SSO dashboard after editing", oldCourseNameOnStudentStatus, "success") +
						report.reportFooter());
						done();
				}
			});
		...
		it(". Validate 'Drop Course' option is available in the profile drop-down list", function(done) {
		...
		it(". Validate dropped course is not present on student dashboard", function(done) {
			instructorGradebookForDropStudent.getTextIfNoCourseIsPresent(browser).then(function() {
				instructorGradebookForDropStudent.checkIfCourseIsPresentAfterDropedStudent(browser).then(function(courseStatus) {
					if (!courseStatus) {
						console.log(report.reportHeader() +
	...
	it(". Logout as an Instructor", function() {
		userSignOut.signOutFromSSO(userType, browser).then(function() {
			pageLoadingTime = 0;
			takeQuizpo.pollingPageLoad(pageLoadingTime, browser, done, "Page is loading");
		});
