...
describe(scriptName + 'CCS/CAS/ASSIGNMENT :: I WILL CHOOSE ASSIGNMENT WITH DROP LOWEST SCORE TEST STRATEGY VALIDATION', function() {
	...
	before(function(done) {
		...
		console.log(report.formatTestScriptFileName("***validateIWillChooseQuestionAndDropLowestStrategyForAssessment.js***"));
	});
	...
	it(". Complete the IWC Assessment form Setting tab for system created assignment", function(done) {
		pageLoadingTime = 0;
	...
	it(". Filter first Question has Type, Bloom, Difficulty Specified", function(done) {
				if(product === "COMM4"){
				var typeFilterText;
				var bloomFilterText;
	...
	it(". Specify the number of questions to be included in the assignment", function(done) {
		browser
		...
		if (product === "\"ORGB5\"" || product === "PFIN6\"" || product === "\"SOC5\"" || product === "\"COMM4\"") {
			...
			assessmentsPage.validatePresenceOfFeedbackColoumn(browser).then(function(length) {
				...
				assessmentsPage.checkIfCorrectIconIsPresentOnHideAnswersPanel(browser).then(function() {
					if (parseInt(feedbackColoumnCount) === assessmentData.systemgenerated.scorestrategyhigh.feedbackColoumnCount) {
						console.log(report.reportHeader() +
	...
	it(". Validate Question, cancel and save/close button under preview tab", function(done) {
	...
	it(". Save the IWC Assessment using save/close button and verify if its saved successfully", function(done) {
		assessmentsPage.clickOnQuestionButtonUnderPreviewTab(browser).then(function() {
	...
	it(". Fetch correct questions from quiz results page", function(done) {
		casPage.getQuestionsCorrect(browser).then(function(questionsCorrect) {
			...
			console.log("Total Assignment Questions " + assessmentData.systemgenerated.iwillchoose.questions);
			studentAssignmentCompletionStatus = "success";
			console.log(report.reportHeader() +
				report.stepStatusWithData("CAS : Student Completed the assignment with questions " + assessmentData.systemgenerated.iwillchoose.questions + " and got a score of  ", questionsCorrect, "success") +
				report.reportFooter());
			done();
		});
	});
	...
	it(". Fetch correct questions from assessment results page", function(done) {
		casPage.getQuestionsCorrect(browser).then(function(questionsCorrect) {
			...
			studentAssignmentCompletionStatus = "success";
			console.log(report.reportHeader() +
				report.stepStatusWithData("CAS : Student Completed the 2nd attempt of assignment with questions " + assessmentData.systemgenerated.iwillchoose.questions + " and got a score of  ", questionsCorrect, "success") +
				report.reportFooter());
			done();
		});
	});
	...
	it(". Fetch correct questions from quiz results page", function(done) {
		casPage.getQuestionsCorrect(browser).then(function(questionsCorrect) {
			...
			studentAssignmentCompletionStatus = "success";
			console.log(report.reportHeader() +
				report.stepStatusWithData("CAS : Student Completed the 3rd attempt of assignment with questions " + assessmentData.systemgenerated.iwillchoose.questions + " and got a score of  ", questionsCorrect, "success") +
				report.reportFooter());
			done();
		});
	});
	...
	it(". Validate due date edited successfully without any error message", function(done) {
		instructorGradebookStudentDetailedPage.dueDateValue(browser, assessmentsPage.getAssignmentName()).then(function(dueDate) {
	...
	it.skip(" [Gradebook] Verify whether the system generated student score(Questions Correct)  on Student Detailed Results Page remains same after attempt override", function(done) {
		this.retries(3);
		if (assignmentCreationStatus === "failure") {
			this.timeout(courseHelper.getElevatedTimeout("timeoutForDependentFeature"));
		}
		// if (process.env.RUN_ENV.toString() === "\"staging\"") {
		instructorGradebookStudentDetailedPage.checkStudentScore(browser, assessmentsPage.getAssignmentName(), averageScoreOnGradebook)
			.then(function() {
				console.log(report.reportHeader() +
					report.stepStatusWithData("GRADEBOOK : Questions correct retrieved from Student Assessment Result page which is " + averageScoreOnGradebook + " questions, is compared against the student score(Questions correct) retrieved from Instructor GradeBook after attempt override", averageScoreOnGradebook, "success") +
					report.reportFooter());
				done();
			});
	});
	...
	it(". Navigate to details of student's attempts GradeBook view", function(done) {
		instructorGradebookNavigationPage.navigateToDetailsOfStudentsAttempts(browser, assessmentsPage.getAssignmentName()).then(function() {
			// instructorGradebookNavigationPage.navigateToDetailsOfStudentsAttempts(browser, "ROBO-Q-U-30 RANDOM NO 357").then(function () {
			done();
		});
	});
	...
	it(". Retrieve Attempts done by student before clearing the attempt", function(done) {
		instructorGradebookStudentDetailedPage.getAttempts(browser).text().then(function(attemptsValue) {
			//console.log("Completed Attempts count before deleting = "+attemptsValue);
			console.log(report.reportHeader() +
	...
	it(". Validating if user clicks on No button on attempt deleting confirmation box then Attempts remain as it is", function(done) { //in process
	...
	it(". Delete confirmation box should hidden from the page", function(done) { //in process
	...
	it(". Get highest percentage value and select that option from select box option", function(done) {
	...
	it(". Validate the number of attempts in drop-down list reduced by 1 after deleting the assignment's attempt", function(done) {
	...
	it(". Validate that after deleting one attempt the attempts count decreased by one at student details assessment result page", function(done) {
	...
	it(". Again click on back button for navigating instructor result page", function(done) {
		instructorGradebookNavigationPage.clickOnGradebookBackButton(browser).then(function() {
			pageLoadingTime = 0;
			takeQuizpo.pollingPageLoad(pageLoadingTime, browser, done, "Gradebook Page is loaded");
		});
	...
	it(". Validate score on main gradebook page", function(done) {
		mainGradebookView.getScoreText(browser, 1).then(function(grades) {
			...
			var roboPoints = assignmentAvgScore.getAvgScoreAfterDropLowestScore(correctQuestionCountFromAssessmentDetailedPage);
