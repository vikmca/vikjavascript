...
describe(scriptName + 'STUDENT :: Flashcards on the title TOC page', function() {
	var browser;
	var allPassed = true;
	var userType;
	var courseName;
	...
	before(function(done) {
		...
		totalTime = 0;
		//Reports
		console.log(report.formatTestName("STUDENT :: Flashcards on the title TOC page"));
	...
	if (stringutil.removeBoundaryQuotes(process.env.RUN_IN_PLATFORM.toString()) == "mobile" || stringutil.removeBoundaryQuotes(process.env.RUN_IN_PLATFORM.toString()) == "iOS") {
		...
		it(". Verify that clicking on Show More button navigates to the StudyBoard page and the Flashcard tab is selected by default", function(done) {
