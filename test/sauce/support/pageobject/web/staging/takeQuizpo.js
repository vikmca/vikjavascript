...
module.exports = {
	...
	retakePraticeQuiz: function(browser) {
		return browser
			.sleep(2000)
			.waitForElementByCss(retakeButtonOnResultPage, asserters.isDisplayed, 10000)
			.click();
	...
	deleteAssignmentsFromBrowser: function(browser, assignmentCount, done) {
		function deleteAssignmentsFromListView() {
			return browser
				.sleep(5000)
				.elementByXPath("(//div[contains(@class,'assignment-list-container')]//td[contains(@data-heading,'Max Points')])[1]").text()
				.then(function(text) {
					var flag = parseInt(text);
					return browser
						.sleep(1000)
						.elementByXPath("(//div[contains(@class,'assignment-list-container')]//td[contains(@class,'link')])[1]").click()
						.sleep(1000)
