...
describe(scriptName + '4LTR (' + 'Instructor/Student' + ') :: TAKE/RETAKE PRACTICE QUIZ METRICS RECORDING ON INSTRUCTOR AND STUDENT CONCEPT TRACKER VALIDATION', function() {
	...
	before(function(done) {
		...
		totalTime = 0;
		//Reports
		console.log(report.formatTestName("ANALYTICS :: TAKE NEW AND PAST PRACTICE QUIZ METRICS RECORDING ON INSTRUCTOR AND STUDENT CONCEPT TRACKER"));
		console.log(report.formatTestData(data.urlForLogin, data.userId, product, courseName));
		console.log(report.formatTestScriptFileName("***takeNewAndPastPracticeQuizAndAnalyticsValidation.js***"));
	});
	...
	it(". Launch the past Practice quiz", function(done) {
	...
	it(". Record sum of Correct answer and Total questions for both quizzes attempted on student concept tracker", function(done) {
		...
		finalSumOfCorrectAnswer = parseInt(preExistingCorrectAnswerAtStudentEnd) + sumOFCorrectAnswerFromTakeAndRetake;
		sumOFTotalQuestionsTakeAndRetake = parseInt(totalQuestionsForTakeQuiz) + parseInt(totalQuestions);
		finalSumOfTotalQuestions = parseInt(preExistingTotalQuestionsAtStudentEnd) + sumOFTotalQuestionsTakeAndRetake;
		console.log(report.reportHeader() +
