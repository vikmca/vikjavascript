...
module.exports = {
    ...
    playVideoOnCarousel: function (browser) {
        return browser
            .waitForElementByCss(".fourltr-video.figure.fixed video", asserters.isDisplayed, 20000)
            .getLocationInView()
    ...
    playVideo: function (browser, id) {
        return browser
            ...
            // .execute("document.getElementById('" + id + "').play()")
            .sleep(10000)
            .execute("document.getElementsByTagName('video')[0].pause()");
            // .execute("document.getElementById('" + id + "').pause()");
    },

    playVideoYouTube: function (browser, id) {
            browser.element('iframe', function(err, el) {
                browser.frame(el.value.ELEMENT, function() {
                    browser.getHTML('button', function(err2, html) {
                        console.log(html);
                        done();
                    });
                });
    ...
    fetchVideoCaption: function(browser){
      return browser
          .hasElementByCssSelector(".owl-item.active .vjs-captions-button.vjs-menu-button").then(function(element){
              console.log("element"+element);
            if(element){
            return  browser
                .waitForElementByCss(".owl-item.active .vjs-captions-button.vjs-menu-button", asserters.isDisplayed, 60000)
                .text();
            }
            else{
              console.log(report.reportHeader() +
   ...
   clickOnOnTranscriptButton: function (browser){
    if (process.env.RUN_IN_BROWSER.toString() === "\"internet explorer\"") {
     ...
     return browser
         .sleep(3000)
         .execute("return (document.evaluate(\"//button[contains(@id,'topTranscriptButton')]\", document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue).scrollIntoView()")
         .sleep(1000)
         .execute("return window.scrollBy(0,-180)")
         .sleep(1000)
         .waitForElementByCss("button[id*='topTranscriptButton']", asserters.isDisplayed, 90000)
         .click();
      }else{
